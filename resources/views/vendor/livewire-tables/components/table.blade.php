...
@elseif ($component->isBootstrap())
    <div wire:key="{{ $tableName }}-twrap"
        {{ $attributes->merge($customAttributes['wrapper'])
            ->class(['table-responsive scrollbar ms-n1 ps-1' => $customAttributes['wrapper']['default'] ?? true])
            ->except('default') }}
    >
        <table
            wire:key="{{ $tableName }}-table"
            {{ $attributes->merge($customAttributes['table'])
                ->class(['laravel-livewire-table table table-sm fs-9 mb-0' => $customAttributes['table']['default'] ?? true])
                ->except('default')
            }}
        >
            <thead
