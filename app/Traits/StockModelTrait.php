...
trait StockModelTrait
{
    ...
    // ✅ Wholesales status accessor
    public function getWholesalesStatusAttribute()
    {
        return (bool) optional($this->wholessales_stock_prices)->status;
    }

    // ✅ Retail status accessor
    public function getRetailStatusAttribute()
    {
        return (bool) optional($this->supermarkets_stock_prices)->status;
    }
    ...
    protected static function boot()
    {
        ...
        // Auto-generate SEO slug after a new product is created.
        // Runs AFTER insert so the ID is already available.
        static::created(function (Stock $stock) {
            if ($stock->name) {
                StockSeoService::applyToStock($stock);
            }
        });

        // Regenerate SEO slug whenever the product name changes on update.
        static::updating(function (Stock $stock) {
            if ($stock->isDirty('name') && $stock->name) {
                $stock->seo = StockSeoService::generateSlug($stock->name, $stock->id);
            }
        });
    }
